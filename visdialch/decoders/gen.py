...
class GenerativeDecoder(nn.Module):
    def __init__(self, config, vocabulary):
        ...
        self.lstm_to_words = nn.Linear(self.config["lstm_hidden_size"],
                                       len(vocabulary))
    ...
    def forward(self, encoder_output, batch):
        ...
        if self.training:
            ...
            ans_len = batch["opt_len"]

            # shape: (batch_size * num_rounds * num_options, word_embedding_size)
            ans_in_embed = self.word_embed(ans_in)

            # reshape encoder output to be set as initial hidden state of LSTM.
            # shape: (lstm_num_layers, batch_size * num_rounds * num_options, lstm_hidden_size)
            ...
            # shape: (batch_size * num_rounds * num_options, max_sequence_length, lstm_hidden_size)
            ans_out, (hidden, cell) = self.answer_rnn(ans_in_embed, (init_hidden, init_cell))

            # shape: (batch_size * num_rounds * num_options, max_sequence_length, vocabulary_size)
